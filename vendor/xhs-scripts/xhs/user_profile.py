...
def get_my_profile(page: Page) -> UserProfileResponse:
    """获取当前登录用户自己的主页信息（无需提供 user_id / xsec_token）。

    利用 my_notes._resolve_my_profile_url 自动解析真实主页地址，
    再提取完整的 UserProfileResponse（资料 + 互动数 + 笔记列表）。

    Raises:
        RuntimeError: 未登录或数据提取失败。
    """
    from .my_notes import _resolve_my_profile_url
